                                    ...
                                    <tr class="hover:bg-gray-50 transition ungraded-row">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex flex-col gap-2">
                                                ...
                                                @endif
                                                <button type="button" 
                                                    onclick='openGradingModal(@json($submission), false)'
                                    ...
                                    <tr class="hover:bg-gray-50 transition graded-row">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex flex-col gap-2">
                                                ...
                                                @endif
                                                <button type="button" 
                                                    onclick='openGradingModal(@json($submission), true)'
                                                    class="w-full px-3 py-1.5 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors text-sm font-medium flex items-center gap-2 justify-center">
                                                    <i class="fas fa-edit"></i>
                                                    edit Grade
                                                </button>
...
>       <div id="gradingModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-blue-50 border-l-4 border-blue-600 rounded-lg p-4 mb-6">
                            <h4 class="font-semibold text-gray-800 text-lg mb-2"><i class="fas fa-user text-blue-600"></i> Student Information</h4>
                        ...
                        <div class="bg-purple-50 border-l-4 border-purple-600 rounded-lg p-4 mb-6">
                            <h4 class="font-semibold text-gray-800 text-lg mb-2"><i class="fas fa-file-alt text-purple-600"></i> Assignment Details</h4>
                    ...
                    <div class="bg-green-50 border-l-4 border-green-600 rounded-lg p-4 mb-6">
                        <h4 class="font-semibold text-gray-800 text-lg mb-2"><i class="fas fa-paperclip text-green-600"></i> Submitted Work</h4>
                    ...
                    <form id="gradingForm" action="" method="POST" onsubmit="return handleGrading(event)">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Grade / Score</label>
                                <div class="flex items-center gap-3">
                                    <input type="number" name="grade" id="gradeInput" min="0" step="0.5" class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500" required>
                                    <span class="text-gray-600 font-medium">/ <span id="maxPoints">100</span> Points</span>
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Feedback / Comments</label>
                                <textarea name="feedback" id="feedbackInput" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 resize-none" required></textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Status</label>
                                <select name="status" id="statusInput" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500">
                            ...
                            <div class="flex gap-3 mt-6">
                                <button type="button" onclick="closeGradingModal()" class="flex-1 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg font-semibold">Cancel</button>
                                <button type="submit" id="modalSubmitBtn" class="flex-1 px-6 py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-lg font-semibold">Submit Grade</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        setupSearch('searchUngraded', '.ungraded-row');
        setupSearch('searchGraded', '.graded-row');

        function openGradingModal(submission, isEdit = false) {
            const modal = document.getElementById('gradingModal');
            const form = document.getElementById('gradingForm');
            
            // 1. Update UI and Route based on Mode
            document.getElementById('modalTitle').innerText = isEdit ? 'Edit Grade' : 'Grade Submission';
            document.getElementById('modalSubmitBtn').innerText = isEdit ? 'Update Grade' : 'Submit Grade';
            
            // Match the Laravel Routes provided
            form.action = isEdit 
                ? `/assignments/grade-submission/edit/${submission.submission_id}` 
                : `/assignments/grade-submission/${submission.submission_id}`;
            ...
            document.getElementById('studentInfo').innerHTML = `
                <div class="flex items-center gap-3">
                    <img src="https://ui-avatars.com/api/?name=${encodeURIComponent(submission.student_name)}&background=667eea&color=fff" class="w-10 h-10 rounded-full">
                    <div>
                        <p class="font-bold text-gray-800">${submission.student_name}</p>
                        <p class="text-xs text-gray-500">${submission.student_email}</p>
                    </div>
                </div>
            `;
            ...
            document.getElementById('maxPoints').textContent = assignment.total_points;
            document.getElementById('gradeInput').max = assignment.total_points;
            document.getElementById('gradingModalAssignmentInfo').innerHTML = `
                <p><span class="font-semibold">Assignment:</span> ${assignment.assignment_name}</p>
                <p><span class="font-semibold">Points:</span> ${assignment.total_points}</p>
            `;
            ...
            if (isEdit) {
                ...
                document.getElementById('statusInput').value = submission.grade_status || 'graded';
            } else {
            ...
            let previewHTML = `<p class="text-xs mb-2">Submitted: ${new Date(submission.submitted_at).toLocaleString()}</p>`;
            if (submission.attachment_path) {
                previewHTML += `
                    <div class="flex items-center gap-2">
                        <i class="fas fa-file-pdf text-red-500"></i>
                        <a href="/storage/${submission.attachment_path}" target="_blank" class="text-blue-600 hover:underline text-sm font-medium">View Submission File</a>
                    </div>`;
            }
            if (submission.submission_link) {
                previewHTML += `
                    <div class="flex items-center gap-2 mt-2">
                        ...
                        <a href="${submission.submission_link}" target="_blank" class="text-blue-600 hover:underline text-sm font-medium">View External Link</a>
                    </div>`;
            }
            document.getElementById('submissionPreview').innerHTML = previewHTML;
        ...
        function handleGrading(event) {
            ...
            const maxPointsValue = parseFloat(document.getElementById('maxPoints').textContent);
            const gradeValue = parseFloat(gradeInput.value);

            if (gradeValue > maxPointsValue) {
                alert(`Error: Grade (${gradeValue}) cannot exceed total points (${maxPointsValue})`);
                return false;
            }

            if (confirm('Are you sure you want to save this grade?')) {
                form.submit();
            }
        }
        ...
        function exportToCSV() {
            ...
            rows.forEach(row => {
                ...
                cols.forEach((col, index) => {
                    if (index < cols.length - 1) csvRow.push('"' + col.textContent.trim().replace(/"/g, '""') + '"');
                });
            ...
            a.href = url;
            a.download = `graded_submissions_${new Date().toISOString().slice(0,10)}.csv`;
            a.click();
        }
    </script>
</body>
